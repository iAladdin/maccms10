...
var MacPlayer = {
  GetDate: function (f, t) {
    ...
    f = f.replace(
      /MM/,
      t.getMonth() > 9 ? t.getMonth().toString() : "0" + t.getMonth()
    );
  ...
  GetUrl: function (s, n) {
    return this.Link.replace("{sid}", s)
      .replace("{sid}", s)
      .replace("{nid}", n)
      .replace("{nid}", n);
  },
  ...
  Show: function () {
    ...
    // a.charset = "utf-8";
    // a.src =
    //   base64decode("Ly91bmlvbi5tYWNjbXMuY29tL2h0bWwvdG9wMTAuanM=") +
    //   "?r=" +
    //   this.GetDate("yyyyMMdd");
  ...
  Play: function () {
    document.write(
      "<style>.MacPlayer{background: #000000;font-size:14px;color:#F6F6F6;margin:0px;padding:0px;position:relative;overflow:hidden;width:" +
        this.Width +
        ";height:" +
        this.Height +
        ';min-height:100px;}.MacPlayer table{width:100%;height:100%;}.MacPlayer #playleft{position:inherit;!important;width:100%;height:100%;}</style><div class="MacPlayer">' +
        '<iframe id="buffer" src="" frameBorder="0" scrolling="no" width="100%" height="100%" style="position:absolute;z-index:99998;"></iframe><iframe id="install" src="" frameBorder="0" scrolling="no" width="100%" height="100%" style="position:absolute;z-index:99998;display:none;"></iframe>' +
        '<table border="0" cellpadding="0" cellspacing="0"><tr><td id="playleft" valign="top" style="">&nbsp;</td></table></div>'
    );
    this.offsetHeight = $(".MacPlayer").get(0).offsetHeight;
    this.offsetWidth = $(".MacPlayer").get(0).offsetWidth;
    document.write(
      "<scr" + 'ipt src="' + this.Path + this.PlayFrom + '.js"></scr' + "ipt>"
    );
  },
  Down: function () {},
  Init: function () {
    ...
    if (
      this.Agent.indexOf("android") > 0 ||
      this.Agent.indexOf("mobile") > 0 ||
      this.Agent.indexOf("ipod") > 0 ||
      this.Agent.indexOf("ios") > 0 ||
    ...
    if (MacPlayerConfig.player_list[this.PlayFrom] != undefined) {
      if (MacPlayerConfig.player_list[this.PlayFrom].ps == "1") {
        this.Parse =
          MacPlayerConfig.player_list[this.PlayFrom].parse == ""
